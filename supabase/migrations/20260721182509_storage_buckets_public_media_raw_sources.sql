-- Product media buckets (ADR-020 blob cutover).
...
-- Public read for public-media objects
...
-- Authenticated roles with bb_role in (publication, admin, security) may insert/update/select public-media
...
-- Deny anon on raw-sources explicitly (no SELECT policy for anon on that bucket)
