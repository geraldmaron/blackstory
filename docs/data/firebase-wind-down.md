...
- [x] **Admin re-targeted to Vercel (2026-07-25):** `apphosting.admin.yaml`, `apps/admin/Dockerfile`, `docs/runbooks/admin-cloud-run.md`, and `scripts/apphosting-build.mjs` deleted. Admin is now a standalone Vercel project (own `apps/admin/vercel.json`, `output:'standalone'` removed) deployed from git main exactly like public web — no GCP/Cloud Run/Cloud Build. The Cloud Run + IAP direction was abandoned: it would reintroduce the GCP surface this whole effort removes, on a Supabase+Vercel stack. Admin stays a separate deployable purely for credential isolation — its write-capable `DATABASE_URL` (writes `bb_canonical`/`bb_publication`/`bb_audit`) lives in admin's own Vercel project env, never in the anonymous public site's runtime. Firebase App Hosting backend `black-book-admin-production` still needs console-level decommission by the owner.
...
- [x] **`apphosting.admin.yaml` deleted (2026-07-25).** Admin re-targeted to a standalone Vercel project (see the Admin row above). Remaining owner action is console-level only: create/point the admin Vercel project at `apps/admin`, set its env (write-capable `DATABASE_URL` + Supabase keys from what were the Secret Manager values), and decommission the Firebase App Hosting backend `black-book-admin-production`.
