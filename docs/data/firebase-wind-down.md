...
- [ ] **`apphosting.admin.yaml` NOT deleted.** `docs/runbooks/admin-cloud-run.md` is a prepared-but-not-yet-executed Cloud Run + IAP runbook; no evidence in-repo of a completed deploy/DNS cutover. Firebase App Hosting remains admin's only live deploy path until that runbook's deploy is actually run and verified.
