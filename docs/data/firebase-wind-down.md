...
- [x] Scheduled Cloud Functions runtime retired; Corsair/systemd is the recurring scheduler (ADR-028 is not itself recovered in `docs/decisions-carryover.md`, but its "Small recovered decisions" ADR-018 entry names this same replacement)
- [x] **Public web DNS on Vercel** (hard-cut 2026-07-22; `docs/decisions-carryover.md`, "Small recovered decisions"); public web App Hosting configs **retired in-repo** — Vercel is sole public host
...
- [x] **Discovery Cloud Functions + Scheduler deleted (2026-08-15):** 5 Gen2 functions and 5 Cloud Scheduler jobs removed from `black-book-efaaf`. GitHub Actions (`discovery-campaigns.yml`, ADR-028; see the ADR-018 entry under `docs/decisions-carryover.md`, "Small recovered decisions" for the live successor) is the sole scheduler. `gcloud functions list` / `gcloud scheduler jobs list` / `gcloud run services list` are empty.
...
- [x] Structured SoR is Postgres (`docs/decisions-carryover.md`, "Firestore as system of record, reversed"); Firestore has no live database, rules, or indexes left — repo-side config is fully removed, not just "export/rollback only"
- [x] **`repo-348e.8` (in-repo config removal):** `functions/` (5 Cloud Functions v2 schedules) and `firebase.discovery.json` deleted — the GitHub Actions cron (`discovery-campaigns.yml`, ADR-028; see `docs/decisions-carryover.md`,
"Small recovered decisions", ADR-018 entry, for the live successor) is the sole scheduler now. `.firebaserc` and `infra/firebase/{firebase*.json,firestore*.rules,firestore*.indexes.json,storage.rules}` deleted (no deploy target, no rules/indexes left to deploy). `integration-firebase` CI job and the `migrate-firestore` dry-run jobs in `deploy-staging.yml`/`deploy-production.yml` removed. `packages/firebase/src/rules.integration.test.ts` and `packages/testing`'s Firebase emulator harness deleted (nothing left to test rules against). `@firebase/rules-unit-testing` dropped from `packages/firebase`.
