<!--
  How to seed reference.jurisdictions (state/county FIPS) for geo joins and
...
**Why:** Production had `reference.jurisdictions` empty — geo-integrity and
...
1. **Nation + states (52 rows):** `packages/ops-data/scripts/load-reference-jurisdictions.ts`
2. **Counties (~3,144 in-scope):** `packages/ops-data/scripts/load-reference-counties.ts`
   ...
   - Parser: `packages/ops-data/src/jurisdictions/tiger-gazetteer.ts`
...
```bash
node --conditions development --import tsx packages/ops-data/scripts/load-reference-jurisdictions.ts
```
...
```bash
node --conditions development --import tsx packages/ops-data/scripts/load-reference-counties.ts
...
```sql
SELECT kind, count(*) FROM reference.jurisdictions GROUP BY kind ORDER BY 1;
...
`reference.statistical_series` / `statistical_observations` (migration
...
INGEST_PHASE1_INDICATORS_APPLY=1 DRY_RUN=0 DATABASE_URL=… \
  node --conditions development --import tsx packages/ops-data/scripts/ingest-phase1-indicators.ts
BUILD_PHASE1_COVERAGE_APPLY=1 DRY_RUN=0 DATABASE_URL=… \
  node --conditions development --import tsx packages/ops-data/scripts/build-phase1-indicator-coverage-snapshot.ts
```

The coverage snapshot upserts `published.materialized_snapshots` key `phase1IndicatorCoverage`
