# Incident response and kill switches (BB-035)
...
- Release rollback: BB-019 immutable release pointer and paired search-index version
- Canonical recovery: [`backup-restore.md`](./backup-restore.md) only when release rollback is insufficient
- Audit trail: BB-018 audit/outbox
...
**Goal:** Demonstrate every BB-035 acceptance criterion in staging or a no-cloud simulation. No production mutation is required.
...
3. **00:20 — Latest release contains an unauthorized page.** Operator engages publication and performs the BB-019 atomic active-pointer rollback with the prior search-index version.  
