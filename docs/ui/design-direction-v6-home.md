...
The hero map inside beat 0 is a **Surface-contained live readout**: the persistent MapLibre plate (`MapStage`) is **positioned** into the hero map column (viewport-fixed box from `hero-map-inset.ts`, not `clip-path`) so real archive pins render in-panel — never a decorative sketch, never full-bleed wallpaper under the whole page. The copy column uses a **flat translucent Surface scrim** (`color-mix` Surface/transparent; denser in dark theme) — not an opaque slab — while the map inset stays map-column-only (do not reframe the plate under the copy).
...
- **Record carousel** (primary column): focusable region (`tabindex="0"`), `aria-roledescription="carousel"`.
  ...
  - **Facts strip:** inline Kind / Where / Era / Evidence rows beside map preview (see [`patterns-record-anatomy.md`](./patterns-record-anatomy.md)).
...
- Live hero map wiring: `hero-map-inset.ts` positions MapStage over `.ds-home-hero__map` (map column only). Copy column is a flat translucent Surface scrim (`72%` light / `84%` dark `color-mix`); do not expand the inset under the copy.
