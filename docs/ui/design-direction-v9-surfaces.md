...
**`/history`** dissolves. The decade stepper becomes the Atlas Time panel. The kind composition graph moves to `/data`, which is its only honest home, rather than being deleted. The record list becomes `/records`. `HistoryExperience`, `DecadeStepper`, `HistoryOverviewStrip` and `HistoryResultList` are deleted; `HistoryGraphPanel` moves to `/data`. A bookmarked `/history?decade=1930` lands on `/records?era=1930s` with the era chip active and "See these on the Atlas" one click away.
