...
**First paint is a map, not pins on empty canvas — and it is the plate's own picture.** `/explore` first HTML is the CONUS board in Web Mercator (`/geo/us-conus-mercator.svg`, `conus-mercator.ts`) with state hairlines and the pin plate, laid over the exact frame the plate opens on (CONUS fitted with `PLATE_OPENING_PADDING_PX`, floor at zoom 3) and clustered with MapLibre's own algorithm and parameters (`first-paint-clusters.ts`), so the board's discs are the plate's discs. An Albers board could never overlay a Mercator canvas — the 49th parallel arcs a tenth of the board's height in one and is straight in the other — and read as an old map replaced by a new one on every load. MapLibre starts as ocean-only; hiding the board when `.maplibregl-canvas` appears leaves a blank plate. Hand off when `.ds-map-stage[data-plate-ready]` is set: the first `idle` after style apply, or when state polygons land, whichever is first; the stage also fires `ready`, and the establishing shot (`useAtlasCamera`) starts a beat after that, on the revealed plate, never under the crossfade. After handoff, camera moves, uncovered pin and cluster clicks, and MapLibre pan/zoom own the plate. If WebGL never arrives, the board stays and still takes wheel, drag, pinch, and pin clicks. Pin discs on that locator are hit targets: a tap that does not pan opens the record sheet (opaque `pin-N` ids match catalog geography; shop tokens stay off the first document). Majority record discs are Page Sand on Door, Explore, MapFrame, and the live plate; copper stays on holding walks and the selected pin. A pin or cluster click opens the record sheet on the plate (not a place or entity page). Cluster clicks resolve one leaf so a national aggregate still opens a card.
