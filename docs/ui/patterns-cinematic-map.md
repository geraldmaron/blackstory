...
| **Engaged** (hands-on) | The reader taps **Explore the map**. The scrim lifts, editorial content and footer recede, the full surface takes touch. Site primary nav stays visible for wayfinding. A single **✕ / Close** relocks and restores the content. | Live (pan/zoom/select) | Suspended while engaged |
...
**Engaged recedes editorial chrome, not primary nav.** `CinematicMapProvider` mirrors Engaged onto `<html>` as `data-cinematic-engaged`, and `cinematic-map.css` keys off `:root[data-cinematic-engaged]` to fade + `visibility: hidden` the site footer, anything the route marks `data-cinematic-recede`, and the route's `edition-atmosphere` wash. The site header (`.ds-shell-header`) stays visible and interactive so wayfinding survives Engaged. Document scroll suspends; `.ds-cinematic-rail` pins below `--ds-island-clearance`. Explore's decade dock is in the Engaged hit list (`pointer-events: auto`, above the rail). Fading only `.ds-cinematic-content` without document-level recede leaves surrounding page chrome legible over a live map, which reads as "the map zoomed in on top of the page" rather than a transition into it.

**The plate's geometry change is animated, and the camera is fitted after it lands.** `animateHeroMapPlate()` (`hero-map-inset.ts`) transitions the plate box between hero-inset and full-bleed, resizing the GL buffer every frame so the canvas does not stretch, and runs the camera fit only once the box has stopped moving — a fit computed mid-flight lands off-center. Reduced motion skips straight to the destination.
