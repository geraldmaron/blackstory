...
instead of (or around) a single folded chapter.
...
- **Forbidden words/patterns**, per the user's standing global style rules
  (`~/.claude/CLAUDE.md`) and `voice-theme-chapters.md` Rule 3:
  - Hype words: "seamless," "robust," "revolutionary," "best-in-class," and
    similar, unless defined and earned.
  - Spaced em dashes — use periods, commas, colons, or parentheses instead.
  - Uplift/perseverance clichés: "against all odds," "refused to give up," "a
    testament to the human spirit," or any phrasing that resolves hardship
    into a moral the record hasn't earned.
...
- Every era's numbers get an attached **Method note** — a collapsible block,
  not inline prose — covering: what the spine segment actually measures,
  any real gap in coverage for that span, and any unreconciled seam between
...
## 5. Worked example — "Buying a home," era 1955 → present

This example uses real spine values pulled from the dev DB at the time of
writing (obs ids in the trailing comment). It is illustrative of register and
structure, not a finished packet section — a real packet would cite a named
primary-document artifact for the FHA/HOLC quote rather than the
representative language shown here.
...
> If you are white, the rate is closer to 55 in 100 — the gap already
> twenty points wide before you've signed anything. The wealth behind those
> two numbers is even further apart: national estimates for the decade put
> a white family's wealth at roughly eight times a Black family's, benchmark
> to benchmark.
>
...
<!--
Sourced 2026-07-26 from bb_reference.statistical_observations, dev DB:
- obs:census-decennial-homeownership-black-nation:nation:US:1950 (34.9%)
- obs:census-decennial-homeownership-white_nh-nation:nation:US:1950 (55.1%)
- obs:dkks-wealth-ratio-white-black-nation:nation:US:1959 (8.11)
- obs:acs-homeownership-rate-black-nation:nation:US:2024 (45.4%)
- obs:acs-homeownership-rate-white_nh-nation:nation:US:2024 (74.0%)
- obs:scf-wealth-ratio-white-black-nation:nation:US:2022 (6.35)
...
| Spine segment structure, seam checks | `bb_reference.spine_series` / `spine_segments` (see migration `20260725000000_spine_series.sql`) |
