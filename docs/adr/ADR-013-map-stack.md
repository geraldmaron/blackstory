...
- **State attribution is an approximate bounding-box test, not polygon geometry.** `findUsStateForPoint` in `us-geography.ts` checks each state's rectangular bounding box (smallest-area-first, so small states like D.C. resolve ahead of larger overlapping neighbors). Dense coastal-metro overlaps remain a known gap in the pure bbox model — historically Manhattan fell inside NJ's rectangle. Mitigation shipped in-repo: (1) NY/NJ Hudson carve-out so east-of-Hudson + Staten Island resolve to NY; (2) `buildMapSource` prefers editorial `jurisdictionLabel` via `findUsStateFromJurisdictionLabel` whenever the release projection carries one. Survey-grade attribution still needs polygon boundary data (e.g. Census TIGER) this bead deliberately does not vendor.
