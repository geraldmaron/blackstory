...
- **Implements toward:** BB-004, Tranche 3 (BB-021–036)
...
9. **Volume ≠ truth**: submissions and repeated mirrors do not auto-increase confidence or publish (BB-032).
...
4. Kill switches exist per feature class (submissions, search, geocode, adapters, research, LLM, uploads, publication, exports) without defaulting to wiping the public corpus (BB-035).
...
| Shared admin credentials in client bundles | Trivial extraction; fails BB-027. |

## Consequences

- BB-004 must encode these assumptions as testable abuse cases.
- Feature design starts with kill switches, quotas, and promotion gates, not afterthought hardening.
- Observability and IR runbooks are launch gates (BB-034, BB-035, BB-063).
- Scaffold code must not temporarily “wire admin into public” for demos.

## Migration triggers

- Amend this ADR when BB-004 discovers a threat class not covered here.
