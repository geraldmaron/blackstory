...
| Migrations / schemas | Boundary stubs only; **no further SQL migrations** | Would have been BB-013+ |
| Backups / PITR | N/A for Cloud SQL this phase | BB-020 if Postgres returns |
...
4. Schema and role boundaries separate public / research / publication / audit access (BB-013; ADR-003, ADR-009).
...
- Connection pooling, statement timeouts, and role credentials are mandatory before public traffic (BB-012, BB-026).
- Application code must never assume public clients can open DB connections.

## Migration triggers

- Introduce a dedicated search cluster only after Postgres FTS/PostGIS proven insufficient under BB-026 / BB-049 / BB-059.
- Split read replicas or regional instances when latency or failover SLOs require it (not a new microservice).
- Engine major upgrades follow Cloud SQL maintenance windows with restore rehearsal (BB-020 / BB-061).

## Rollback considerations

- Schema migrations must be expandable/contractible: deploy compatible SQL before incompatible app code (ADR-006).
- Point-in-time recovery and release-manifest restore are the primary data rollback paths (BB-020), not “drop Cloud SQL.”
