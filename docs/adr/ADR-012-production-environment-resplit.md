...
- **Implements toward:**  (production cloud apply),  (pre-launch resilience)
...
| Local dev | `demo-repo` emulators | Unchanged |
...
in front of it. The only IAP design written for this repo, `infra/gcp/iap/README.md`, assumes
...
- **Admin console runs as a plain Cloud Run service behind Identity-Aware Proxy, with IAP attached
  ...
  (`infra/gcp/iap/README.md`, ), which assumed the load-balancer pattern because that was the
...
`demo-repo` Firestore/Auth/Storage emulators remain the only local development target. No
...
Extends `infra/gcp/wif/` rather than duplicating it. One Workload Identity Pool, hosted in
...
The repository + ref pinning was already correct for production in the  design
...
without an org. This repeats the exact situation  already documented and solved the same way:
...
Restates `docs/security/environment-isolation.md`'s  acceptance invariants for the three-project
...
- **AC-ISO-1 — Development credentials cannot access production.** Now a **real project-separation
  claim** again (D-013 had marked this N/A). `demo-repo` emulators remain local-only;
...
|-------------|---------------|
| Four-project split (restore the original `blackbook-dev`/`staging`/`prod`/`research-prod` target verbatim) | A cloud `dev` project buys little over `demo-repo` emulators, which already give zero-cost, zero-blast-radius local development; a fourth project is ongoing IAM/billing surface for a workflow that doesn't need cloud credentials. Re-evaluate only if cloud-based development or destructive integration testing against realistic data becomes required (unchanged trigger from D-013/ADR-009). |
...
- `infra/gcp/iap/README.md` and `infra/gcp/iap/admin-iap-policy.json` still describe the
  ...
  this ADR specifies. That directory is outside this bead's file ownership ( owns
  ...
  's own docs (`docs/security/service-surfaces.md`, `infra/gcp/isolation-matrix.json`) already
  said "Cloud Run + IAP" for admin and required no correction on that point.
  `docs/security/admin-identity.md` is also unaffected at the application-authorization layer
...
- `workers/submissions-puller` (the pull-based internal ingestion worker) is a new logical surface this
  ...
  (`workers/` is outside 's file ownership). A follow-up bead must implement it; until it exists,
...
- `apps/admin`'s SA moves projects; any hardcoded `black-book-efaaf` reference in admin's own runtime
  config (outside this bead's file ownership) will need updating when  actually applies this.
...
for "before anything is live." Reversing after  applies this would mean either a live data
migration back into one project or accepting permanent three-project overhead; reversing **before**
 applies it costs nothing beyond discarding unapplied Terraform.
...
- `infra/gcp/wif/` — extended, not duplicated, for per-project deploy identities
