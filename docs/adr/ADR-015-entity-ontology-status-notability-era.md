...
| Era/decade model | `era` is a free string in the web seed (`apps/web/src/data/public-seed.ts`); decades are computed only inside `packages/firebase/src/embeddings/text.ts`'s local `deriveEraBucket` | `datePrecision` + `deriveEraBuckets` defined once in `@black-book/domain`, consumed everywhere |
...
| A new package for the ontology (instead of extending `entity.ts`) | The substrate (`CanonicalEntity`, kind field bags, `EntityKind`) already lives in `@black-book/domain`; a parallel package would either duplicate `EntityKind`/`CanonicalEntity` or force every consumer to depend on two packages for one entity model. Extending `entity.ts` in place (plus small, purpose-named sibling modules `entity-status.ts`, `era.ts`, `movement.ts` — matching the existing `school.ts`/`specialized.ts` split, not one giant file) keeps one entity model, one import. |
...
- `packages/firebase/src/embeddings/text.ts`'s `deriveEraBucket` now delegates to
  `@black-book/domain`'s `deriveEraBuckets`; its `resolveEntityYearSpan`'s school case reads
...
- BB-087 (law badges), BB-086 (FactRecord spec), BB-092 (history graph substrate), and BB-095
  ...
  (`@black-book/domain`'s `LAW_STATUSES`, `DatePrecision`/`deriveEraBuckets`,
