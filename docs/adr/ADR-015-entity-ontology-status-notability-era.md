...
| Aspect | Today (verified, pre-) | Target (this bead) |
|--------|------------------------------|---------------------|
...
| Sensitivity | No schema at all | Schema-only `sensitivity[]` classification; presentation is  |

## Problem

`CanonicalEntity` already carries `mergeState` — a lifecycle for *merge* status — but
...
institution, law) currently has no status field at all; 's law badges and 's
...
-  already ships `CanonicalEntity` (11 kinds), `EntityAlias`/`EntityLocation`/
...
-  (living-person compliance) already defines `LivingStatus` and `treatAsLiving()` —
  unknown status is treated as living. Person status in this bead must derive from that single
  source, not duplicate it.
-  (deterministic relevance engine) already runs 7 sequential gates
...
-  (historic safety and place-context engine, still open) needs its own time-scoped,
...
```mermaid
flowchart LR
  A[CanonicalEntity today: 11 kinds, mergeState only] --> B[ decision]
...
1. **Kind-specific status vocabularies, not one global enum.** `place`/`school`/`organization`/
   ...
   `in_force | amended | repealed | struck_down | enjoined` (the exact vocabulary  imports);
...
2. **`statusHistory` is entity-lifecycle status only — never area/condition designations.**
   ...
   (`validTo` omitted/null) record; `statusAsOf()` answers point-in-time queries. 's own
   sundown-town/redlining-grade/exclusion-infrastructure layer records are explicitly **not**
   this field — they get their own, separately-typed record in 's territory, following the
...
3. **Notability is an auditable basis record, never a score.** `NotabilityBasisRecord` is
   ...
   **8th** `RelevanceGateId` (`notability_basis`,
   ...
   for 's methodology section. The cultural-figure calibration ships as
   ...
   This calibration is pending owner ratification before  auto-derives candidate notability
   from it.
...
5. **Sensitivity is schema-only in this bead.** `EntitySensitivity =
   ...
   content warnings, UI treatment) is explicitly 's job. This is a distinct concern from two
   similarly-adjacent ones that must not be conflated with it:  living-person compliance
   (privacy/consent for the living) and location sensitivity classes ('s own place-condition
   designations).
...
against the  compliance lane — exactly the kind of dual-source-of-truth bug this decision
prevents by construction (`personStatusFromLiving` is the only path). Status is a *history* of
time-scoped records, not a scalar, because both the decade/all-time graph views and
's own sundown modeling need "status as of decade D" answered, and a scalar structurally
...
- **Wiring point, documented but not live-wired**: the real projection/release build
  ...
  boundary (it belongs to the publication/release machinery landed under ), so this bead
...
- Two files outside this bead's primary ownership list needed a single-key mechanical rename to
  ...
  unchanged).
-  (law badges),  (FactRecord spec),  (history graph substrate), and
...
harder-to-reverse piece is the cultural-figure `icons_and_firsts_only` calibration once
...
ratifying the calibration (or narrowing/widening it) before  lands is the intended checkpoint.
