...
| Persistent cache | None | SQLite cache (engine per ADR-021) with the caching policy in this ADR layered on top |
...
| AsyncStorage/MMKV as the primary cache store | ADR-021 already selects SQLite; key-value stores don't fit release-stamped, LRU-evicted, size-capped structured cache and would fragment the storage story. |
