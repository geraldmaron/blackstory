...
`unfit` is a floor of zero rather than a small number on purpose, so ten unfit sources cannot accumulate into a supported claim. The resolver also throws on an unrecognized source or assertion class instead of defaulting: under the old lookup a misspelled class silently scored 0.2, which turned a typo into a quiet downgrade rather than a failure.
