 ...
 * `docs/decisions-carryover.md`, "Mobile stack" (MOB-002) — the ADR that
 * decision came from was removed in the 2026-07-24 docs purge.
 *
 ...
 *
 * Per `docs/decisions-carryover.md`, "Mobile stack" (native directories),
 * `ios/` and `android/` are NOT committed — they are regenerated by
 * `expo prebuild` (Continuous Native Generation) from this file plus config
 * plugins. This file is the single source of truth for native identity, never
 * a checked-in native project. `apps/mobile/.gitignore` is the only thing
 * enforcing that; no CI job checks it.
 */
...
const config: ExpoConfig = {
  ...
  // EAS Update runtime-compatibility fence (ADR-024 §2; restated in
  // `docs/decisions-carryover.md`, "Mobile cache and OTA release"): a JS
  // bundle only ever
  ...
  ios: {
    ...
    // from SDK 54's ~15.1, per `pod install` evidence — several Expo-bundled
    // pods, including `expo` itself, require iOS 16.4), which is *above* the
    // originally proposed 16.0 floor. Whichever SDK MOB-006 actually pins
    // governs, so the platform floor wins: 16.4, not 16.0. That check was run
    // against SDK 56; the app now runs SDK 57 and nothing asserts this value.
  ...
  android: {
    // Native minimum OS floor per `docs/decisions-carryover.md`, "Mobile
    // stack" (OS floors): Android 8.0 (API 26)+.
  ...
  plugins: [
    ...
    [
      ...
      // "Mobile stack": OS floors); iOS's floor is set directly via
      // `ios.deploymentTarget` above (a native ExpoConfig field with no
      // plugin needed).
  ...
  extra: {
    ...
    // Explore basemap (`docs/decisions-carryover.md`, "Explore basemap and live map source"):
    // OpenFreeMap by default (web parity). Optional self-hosted PMTiles via MAP_PMTILES_URL.
    // Kill-switch: MAP_BASEMAP_ENABLED=false.
    map: {
