...
Client-side wiring for EAS Update (the OTA mechanism ADR-024 §2 decided) and the
record of the code-signing posture decision ADR-024's 2026-07-20 adversarial
...
**Which ADR-024, and where to read it.** The document is "Mobile build, distribution, OTA update,
and release-rollback model". It was removed in the 2026-07-24 `docs/adr/` purge and is restated in
`docs/decisions-carryover.md`, "Mobile cache and OTA release", which keeps the original section
attributions so the `§N` references below still resolve. Every citation in this file read "ADR-023"
until 2026-09-13 (repo-7u17u): that was the number this decision carried before commit `3b365d44`
shifted the mobile ADRs up by one, and it now belongs to the cache/offline decision, which is not
this one.

## The decision (already made — this bead implements it)

ADR-024's "Adversarial review disposition" (2026-07-20) and
`docs/mobile/security/threat-model.md` T6 already resolved
...
> project's free-tier-first posture (ADR-024 §7). The T6 blast-radius controls
...
> signing later is a **cost-gated upgrade trigger** (ADR-024 §7): adopt it when
...
Re-litigating it requires a new decision record, not a code change here.

## What this bead wired

| Change | File | Why |
|---|---|---|
| `expo-updates` installed | `package.json` / `package-lock.json` (`npx expo install expo-updates --npm`) | ADR-024 amendment #2: the package was entirely absent — there was no OTA mechanism to speak of, only a documented intent. |
| `runtimeVersion: { policy: 'appVersion' }` | `app.config.ts` | ADR-024 §2's structural OTA/rebuild fence: a JS bundle only installs onto a binary whose runtime version matches, so an incompatible OTA is rejected client-side, not shipped-and-crashed. Declaring this costs nothing and does not depend on an EAS project existing. |
...
| `channel` per build profile | `eas.json` (already present, unchanged) | `development`/`preview`/`production` build profiles already each declare a distinct `channel`, matching ADR-024 §1's "OTA can never cross an environment boundary" requirement. Verified correct as-is; nothing to fix. |
...
Per ADR-024 §5 and threat-model T6, with end-to-end code signing off on the
...
## Rollback runbook (ADR-024 §2/§6 — drilled at MOB-021, documented here)
...
4. Clients recover on their **next update check** — recovery latency is
   ...
   (ADR-024 §6 requires this be **drilled**, not merely asserted).
...
to a store binary. That falls to ADR-024 §6's binary contingency — halt the
...
- **EAS Update code signing** — the paid-plan feature itself. Enabling it is
  the recorded cost-gated upgrade trigger (ADR-024 §7), not something to
