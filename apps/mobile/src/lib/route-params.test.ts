...
describe('parseFilterState', () => {
  ...
  it('parses tone, theme, status, confidence, and state when valid', () => {
    expect(parseFilterState({ tone: 'plantation' })).toEqual({ tone: 'plantation' });
    expect(parseFilterState({ theme: 'education' })).toEqual({ theme: 'education' });
    expect(parseFilterState({ status: 'historic' })).toEqual({ status: 'historic' });
    expect(parseFilterState({ confidence: 'high' })).toEqual({ confidence: 'high' });
    expect(parseFilterState({ state: 'dc' })).toEqual({ state: 'DC' });
  });

  it('discards invalid extended filter params', () => {
    expect(parseFilterState({ tone: 'alarm-red' })).toEqual({});
    expect(parseFilterState({ theme: 'not a valid id!!' })).toEqual({});
    expect(parseFilterState({ status: 'pending' })).toEqual({});
    expect(parseFilterState({ confidence: 'extreme' })).toEqual({});
    expect(parseFilterState({ state: 'ZZZ' })).toEqual({});
    expect(parseFilterState({ tone: 'all' })).toEqual({});
  });

  it('round-trips active filters through filterStateToRouteParams', () => {
    const filters = {
      kind: 'places' as const,
      era: '1950s',
      tone: 'massacre' as const,
      theme: 'education',
      status: 'historic' as const,
      confidence: 'high' as const,
      state: 'DC',
    };
    expect(filterStateToRouteParams(filters)).toEqual({
      kind: 'places',
      era: '1950s',
      tone: 'massacre',
      theme: 'education',
  ...
  it('exports closed allow-lists for extended facets', () => {
    expect(MAP_SEMANTIC_TONES).toContain('plantation');
    expect(CONFIDENCE_TIERS).toContain('unrated');
    expect(FILTER_STATUS_VALUES).toContain('in_force');
  });
});
