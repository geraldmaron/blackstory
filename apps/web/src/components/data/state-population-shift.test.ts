...
const SAMPLE_CHANGES: readonly StateChangeLike[] = [
  { stateFips: '06', blackAbsoluteChange: 200_000, shareChangePp: 0.2, blackPopulationTo: 2_200_000 },
  { stateFips: '72', blackAbsoluteChange: -15_000, shareChangePp: -0.4, blackPopulationTo: 900_000 },
  { stateFips: '48', blackAbsoluteChange: 500_000, shareChangePp: 0.3, blackPopulationTo: 3_500_000 },
];

test('buildStateShiftBarRows resolves territory names and sorts by absolute change', () => {
  const rows = buildStateShiftBarRows(SAMPLE_CHANGES, { '72': 'Puerto Rico', '06': 'California', '48': 'Texas' });
...
test('divergingBarDomain returns at least 1 for empty magnitudes', () => {
  ...
  assert.equal(divergingBarDomain(SAMPLE_CHANGES.map((row) => ({
