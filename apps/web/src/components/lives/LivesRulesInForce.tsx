...
const SCOPE_LABEL: Readonly<Record<LivesRule['scopeLevel'], string>> = {
  federal: 'Federal',
  state: 'State',
  local: 'Local',
};
...
/**
 * Laws and rulings in force for this place and decade. Shown beside the measured conditions, not
 * as their cause; the fixed disclaimer always renders, even when no rule is listed.
 */
    ...
    <section className="lives-rules" aria-labelledby="lives-rules-heading">
      ...
      {decade.rulesInForce.length === 0 ? (
        <p className="lives-rules__empty">
          No sourced rules are recorded for this place and decade yet.
        </p>
      ) : (
          ...
          {[...applying, ...others].map((rule) => (
            ...
            >
              ...
              <p className="lives-rule__meta">
                {SCOPE_LABEL[rule.scopeLevel]} ·{' '}
                {formatInForceYears(rule.inForceFromYear, rule.inForceToYear)}
              </p>
              <p className="lives-rule__applies">
                Applied to: {rule.appliesTo.map((slice) => LIVES_GROUP_LABELS[slice]).join(', ')}.{' '}
