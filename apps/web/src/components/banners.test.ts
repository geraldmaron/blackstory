 ...
 * Mirrors the render-to-static-markup pattern used by @blap/ui's semantics.test.ts.
 */
...
test('SensitivityContextBanner never renders a suppression/hide affordance — it is additive markup only', () => {
  ...
  // `aria-hidden`/`bp-visually-hidden` those are legitimate, unrelated accessibility markup
