...
describe('buildExternalMapsSearchUrl', () => {
  it('prefers lat/lng over a prose query', () => {
    ...
    assert.equal(
      url,
      'https://www.google.com/maps/search/?api=1&query=38.9072%2C-77.0369',
    );
  });
  ...
  it('falls back to an encoded query when coordinates are absent', () => {
    ...
    assert.equal(
      url,
      'https://www.google.com/maps/search/?api=1&query=Indianapolis%2C%20IN',
    );
  });
...
describe('externalMapsLinkLabel', () => {
  it('names the destination for screen readers', () => {
    assert.equal(externalMapsLinkLabel('Bethel A.M.E. Church, Indianapolis'), 'Open Bethel A.M.E. Church, Indianapolis in maps');
  });
});
