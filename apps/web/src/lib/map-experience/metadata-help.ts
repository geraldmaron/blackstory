...
export type MetaFieldKey =
  'kind' | 'tone' | 'era' | 'theme' | 'confidence' | 'evidence' | 'where' | 'status';
...
const FIELD_HELP: Readonly<Record<MetaFieldKey, string>> = {
  ...
  tone: 'Historical tone from the Color key: massacre or atrocity, plantation, or Black epicenter. Shade only; the kind shape stays the same.',
