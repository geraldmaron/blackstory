/**
 * Kind + semantic-tone encoding for the explore map. Every entry pairs a shade
 ...
 * kind shade at paint time while keeping the kind glyph.
 */
...
export const MAP_SEMANTIC_TONE_ENCODING: Readonly<Record<MapSemanticTone, KindEncodingEntry>> = {
  massacre: { shade: DIGNITY_PALETTE.kindMassacre, glyph: 'diamond', label: 'Massacre / atrocity' },
  plantation: { shade: DIGNITY_PALETTE.kindPlantation, glyph: 'square', label: 'Plantation' },
  epicenter: { shade: DIGNITY_PALETTE.kindEpicenter, glyph: 'circle', label: 'Black epicenter' },
};
...
export function semanticToneEncodingFor(tone: string): KindEncodingEntry | undefined {
...
export const SEMANTIC_TONE_ENTRIES: ReadonlyArray<readonly [tone: MapSemanticTone, entry: KindEncodingEntry]> = (
  Object.keys(MAP_SEMANTIC_TONE_ENCODING) as MapSemanticTone[]
).map((tone) => [tone, MAP_SEMANTIC_TONE_ENCODING[tone]] as const);
