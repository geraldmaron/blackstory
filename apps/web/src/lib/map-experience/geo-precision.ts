 ...
 * `@black-book/security`'s redaction policy) to the BB-091 `GeoPrecisionTier` display-radius
 ...
 * is the one place BB-051 translates between them — no other file in this bead's scope should
 ...
 * This is the ONLY locality bbox this module resolves without a real BB-091 jurisdiction polygon;
 * every other state/locality combination fails closed (see `resolveDisplayRadiusMeters` below)
 * rather than guessing, matching BB-091's fail-closed radius policy (`precision.ts` module doc).
 */
