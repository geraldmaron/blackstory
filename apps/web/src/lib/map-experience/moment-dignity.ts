/**
 * Whether a map moment renders as PLATE · STILL rather than PLATE · LIVE.
 *
 * Plainness must not be a judgement each author makes per page: a moment about violence marked
 * "not plain" by a forgetful call site is a dignity regression. A moment's plainness is derived
 * from its subject's violence-adjacency, using the same
