  ...
  const handleClick = (event: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    const settle = () => {
      ...
      if (!atRest && !atPageEnd && attempts < SETTLE_ATTEMPTS) {
        // Snap rather than animate on the retry: the reader has already watched
        // one scroll, and a second animation reads as the page fighting them.
        target.scrollIntoView({ behavior: attempts === 1 ? 'smooth' : 'auto', block: 'start' });
        schedule();
        return;
      }
