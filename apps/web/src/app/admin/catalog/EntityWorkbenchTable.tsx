...
import type { CatalogDecisionAction } from '../../../admin/catalog/catalog-decisions-store';
import type { EntityRow, EntitySortKey } from '../../../admin/lib/entity-query';
...
type Props = {
   ...
   * they hold. Hiding the affordance is presentation — `/admin/catalog/merge` re-checks the role.
   */
  ...
  const columns: readonly DataTableColumn<EntityRow>[] = [
    {
      ...
      cell: (row) => (
        <>
          <Link href={`/admin/catalog/${row.id}`} className="story-review__row-title">
            {row.displayName}
          </Link>
  ...
  return (
    <>
      ...
      <DataTable
        ...
        rowHref={(row) => `/admin/catalog/${row.id}`}
        emptyState={
          <p className="ds-sans">
            No entities match this filter. <a href="/admin/catalog">Clear filters</a> to see all{' '}
      ...
      >
        ...
        {canMerge && selectedIds.size >= 2 && selectedIds.size <= MAX_MERGE_SELECTION ? (
          <Link
            ...
            href={`/admin/catalog/merge?ids=${[...selectedIds].map(encodeURIComponent).join(',')}`}
          >
