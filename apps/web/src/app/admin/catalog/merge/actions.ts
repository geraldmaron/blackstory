...
import { commitCanonicalWrite } from '@/admin/lib/canonical-write';
import {
  applyEntityMerge,
  readMergeReversalRecord,
  reverseEntityMerge,
} from '@/admin/lib/entity-merge';
import {
  countLeftBehind,
  countMoved,
  describeMerge,
  mergeLedgerIdFor,
  planMerge,
} from '@/admin/lib/entity-merge-plan';
import type { MergeReversalRecord } from '@/admin/lib/entity-merge-plan';
...
export async function mergeEntities(
  ...
  switch (result.status) {
    case 'ok': {
      ...
      revalidatePath(`/admin/catalog/${plan.survivorId}`);
      for (const absorbedId of plan.absorbedIds) revalidatePath(`/admin/catalog/${absorbedId}`);
...
export async function reverseMerge(
  ...
  switch (result.status) {
    case 'ok':
      revalidatePath('/admin/catalog');
      revalidatePath(`/admin/catalog/${record.survivorId}`);
      for (const absorbedId of record.absorbedIds) revalidatePath(`/admin/catalog/${absorbedId}`);
      return {
        status: 'ok',
