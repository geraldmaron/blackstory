/**
 * The record room: the page body every non-place family renders.
 *
 * On the v9 room kit (a Record room with a right rail), dressed in the record chrome
 * (`components/entity/RecordChrome.tsx`): one type scale, one icon language, pills for the facts
 * a reader glances at, tiles for the facts a reader checks, numbered beats for the prose, and an
 * apparatus band for everything that stands behind the page.
 *
 * It lives here rather than in `page.tsx` because Next allows a route file to export only its own
 * known members, and both `/entity/{id}` and `/invention/{slug}` need to render it. An invention
 * that got its own address but a thinner page would have traded one misrepresentation for another.
 */
...
import type { resolvePublicEntityView } from '../../../lib/public-data/source';
...
export async function EntityRecordRoom({ entity }: { readonly entity: PublicEntityView }) {
