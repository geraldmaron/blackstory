...
test('changing a filter resets to page 1, but paging does not', () => {
  ...
  assert.match(toggleFacetHref('/admin/catalog', query, 'kinds', 'person'), /kind=place%2Cperson/);
  assert.doesNotMatch(toggleFacetHref('/admin/catalog', query, 'kinds', 'person'), /page=/);
  assert.match(entityQueryHref('/admin/catalog', query, { page: 8 }), /page=8/);
});

test('toggling an active facet value removes it', () => {
  const query = parseEntityQuery({ kind: 'person,place' });
  assert.match(toggleFacetHref('/admin/catalog', query, 'kinds', 'person'), /kind=place/);
});
