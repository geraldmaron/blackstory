...
const FORBIDDEN_BROWSER_KEYS = [
  ...
  'NEXT_PUBLIC_ADMIN_DATABASE_URL',
] as const;
...
/** Resolves the write-capable canonical Postgres URL (service role / pooler). */
export function resolvePostgresConnectionString(
  environment: Readonly<Record<string, string | undefined>> = process.env,
): string | undefined {
  assertNoBrowserDatabaseCredentials(environment);
  const url = environment.ADMIN_DATABASE_URL?.trim() || environment.ADMIN_APP_DATABASE_URL?.trim();
...
export function getPostgresPool(
  ...
  if (!connectionString) {
    throw new Error(
      'ADMIN_DATABASE_URL or ADMIN_APP_DATABASE_URL is required for canonical admin reads/writes',
    );
  }
  if (!pool) {
