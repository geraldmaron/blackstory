 ...
 * JS bundle nothing, and is rendered through `mask-image` so its color still comes from a theme
 * token instead of being baked in — which is why the shapes below are drawn in flat black at two
 * different alphas. The mask reads alpha, not color: land at 0.32 becomes a wash, the state
...
function draw(
  ...
  const svg = [
    ...
    // Alpha, not color, is the payload: this file is consumed as a CSS mask.
