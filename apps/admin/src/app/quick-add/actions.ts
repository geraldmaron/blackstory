...
/**
 * Server action for the quick-add surface (BB-085). This is a thin caller: it reads form
 * fields, builds an `OperatorIntakeContext`, and calls the real
 * `runResearchIntake`/`createNodeSafeFetchDependencies` from `@black-book/operator-cli` — the
...
import { createNodeSafeFetchDependencies, runResearchIntake } from '@black-book/operator-cli';
