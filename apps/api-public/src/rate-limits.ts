/**
 * Maps public routes to quota policies using one synchronous token bucket per warm server
 * instance. Quotas are not shared across instances and do not provide a global spend ceiling.
 * Vector search also checks its database kill switch; global quotas require a shared
 * asynchronous store across callers.
 */
import {
...
const PUBLIC_PATH_PATTERNS: ReadonlyArray<{ pattern: RegExp; endpointClass: EndpointClass }> = [
  ...
  // Singular `/v1/entity/:id` maps to the same
