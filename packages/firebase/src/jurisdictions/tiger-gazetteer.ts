 ...
 * `jurisdictions` collection county docs (BB-091 acceptance criterion 1).
 *
 ...
 *
 * HOW TO OBTAIN THE REAL FILE (documented for the human operator, per this bead's "no network
 * access required, don't fabricate a download" constraint):
 ...
 * backfilled later from the Census cartographic boundary shapefiles BB-070 already ingests
 * for map tiles (see the ADR's "Rejected: exhaustive... " and the final report's follow-up
 * note) — recomputing that here would duplicate BB-070's tile pipeline, which is out of scope.
 */
 ...
 * same scope line ADR-008 / BB-050 already draw, not something this bead invents.
 */
