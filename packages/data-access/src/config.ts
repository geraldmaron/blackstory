...
export function parseDatabaseConfig(
  ...
  const roleRaw = environment.BLAP_DB_ROLE ?? environment.DATABASE_ROLE;
  if (!roleRaw || !isDatabaseRole(roleRaw)) {
    throw new Error(`BLAP_DB_ROLE must be one of: ${DATABASE_ROLES.join(', ')}`);
  }

  const connectionString = environment.DATABASE_URL ?? environment.BLAP_DATABASE_URL;
