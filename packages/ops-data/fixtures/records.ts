/** Synthetic records for entity identity, evidence rights and contradiction tests. */
import {
  buildGeoPointFields,
...
// `geohashPrefixes`; the record schema infers a mutable `string[]` for the same field.
...
export const seedPlaceHistoricalLocation: EntityLocationDoc = {
  ...
  role: 'historical',
  geometry: {
    // Uses a coarse Washington, D.C. extent because the cited material does not establish the
    // congregation's exact pre-1918 address. See match.notes for the source limitation.
    type: 'BBox',
  ...
  match: {
    ...
    notes:
      'Pre-1918 location undocumented in the cited sources; this is a coarse D.C. ' +
