 ...
 *     packages/ops-data/scripts/ingest-hate-crime.ts \
...
function loadCountyPolys(path: string): CountyPoly[] {
  const geo = JSON.parse(readFileSync(path, 'utf8')) as {
    features: {
      properties: { name: string; stateFips: string; countyFips: string };
...
async function main(): Promise<void> {
  ...
  for (const [label, p] of [
    ['--hate-crime-csv', hateCrimeCsv],
    ['--agencies-json', agenciesJson],
    ['--participation-csv', participationCsv],
    ['--county-geojson', countyGeojson],
  ] as const) {
    if (!p || !existsSync(p)) {
      console.error(`Missing or nonexistent ${label}`);
      process.exit(2);
    }
  }
  ...
  const hcChecksum =
    hateCrimeZip && existsSync(hateCrimeZip) ? sha256Of(hateCrimeZip) : sha256Of(hateCrimeCsv!);
  ...
  for (const c of counties)
    fipsByName.set(`${c.fips5.slice(0, 2)}|${normalizeCountyName(c.name)}`, c);

  const raw = JSON.parse(readFileSync(agenciesJson!, 'utf8')) as Record<
    string,
  ...
  for (const { abbr, countyKey, a } of agencyRecords) {
    ...
    const nameHit =
      !multiCounty && stateFips
        ? fipsByName.get(`${stateFips}|${normalizeCountyName(countyRaw)}`)
        : undefined;
    const pointHit =
      lat !== undefined && lng !== undefined ? locateCounty(counties, lng, lat) : undefined;
    ...
    if (nameHit) {
      fips5 = nameHit.fips5;
      countyName = nameHit.name;
      basis = pointHit
        ? pointHit.fips5 === nameHit.fips5
          ? 'name_match_confirmed_by_point'
          : 'name_match_point_disagrees'
        : 'name_match_only';
      byName += 1;
    } else if (pointHit) {
      fips5 = pointHit.fips5;
      countyName = pointHit.name;
      basis = 'agency_point_in_county';
      byPoint += 1;
    } else unresolved += 1;
    ...
    oriToCounty.set(ori, {
      ...(fips5 ? { fips5 } : {}),
      approximate: multiCounty || basis === 'agency_point_in_county',
    });
  }
  console.log(
    `agencies: ${agencyDocs.length.toLocaleString()} (name ${byName.toLocaleString()}, point ${byPoint.toLocaleString()}, unresolved ${unresolved.toLocaleString()})`,
  );
  ...
  for await (const line of reader) {
    ...
    const biases = cell('bias_desc')
      .split(';')
      .map((b) => b.trim())
      .filter(Boolean);
    for (const b of biases) bump(d.bias, b);
    if (biases.some((b) => b.includes('Anti-Black'))) d.antiBlack += 1;
    for (const o of cell('offense_name')
      .split(';')
      .map((s) => s.trim())
      .filter(Boolean))
      bump(d.offense, o);
    for (const l of cell('location_name')
      .split(';')
      .map((s) => s.trim())
      .filter(Boolean))
      bump(d.location, l);
  ...
  const countyYearDocs: HateCrimeCountyYearDoc[] = [...agg.entries()].map(([id, d]) => {
    ...
    return hateCrimeCountyYearSchema.parse({
      ...
      year,
      incidents: d.incidents,
      antiBlackIncidents: d.antiBlack,
      victimCount: d.victims,
      reportingAgencyCount: d.oris.size,
      approximatedAgencyIncidents: d.approx,
      biasCounts: topN(d.bias, 40),
      offenseCounts: topN(d.offense, 15),
      locationCounts: topN(d.location, 12),
      source: HC_SOURCE,
      sourceUrl: provenanceUrl(HATE_CRIME_KEY),
      retrievedAt: nowIso,
      contentHash: sha256Json(stable).digest,
      datasetChecksum: hcChecksum,
      license: LICENSE,
  ...
  console.log(
    `incidents: ${totalRows.toLocaleString()}  resolved to county: ${resolvedRows.toLocaleString()} (${((resolvedRows / totalRows) * 100).toFixed(1)}%)`,
  );
  console.log(
    `county-year docs: ${countyYearDocs.length.toLocaleString()} across ${new Set(countyYearDocs.map((d) => d.fips5)).size.toLocaleString()} counties`,
  );

  // ── 3. state participation (the coverage denominator) ──────────────────────────────────
  const partLines = readFileSync(participationCsv!, 'utf8')
    .split(/\r?\n/)
    .filter((l) => l.trim());
  ...
  for (const line of partLines.slice(1)) {
    ...
    const stable = {
      id,
      participating: num(get('participation_agencies')) ?? null,
      source: PARTICIPATION_SOURCE,
      datasetChecksum: participationChecksum,
    };
    participationDocs.push(
      ucrStateParticipationSchema.parse({
        id,
        stateName,
        year,
        ...(num(get('total_agencies')) !== undefined
          ? { totalAgencies: Math.trunc(num(get('total_agencies'))!) }
          : {}),
        ...(num(get('participation_agencies')) !== undefined
          ? { participatingAgencies: Math.trunc(num(get('participation_agencies'))!) }
          : {}),
        ...(num(get('participating_agencies_pct')) !== undefined
          ? { participatingAgenciesPct: num(get('participating_agencies_pct'))! }
          : {}),
        ...(num(get('covered_agencies')) !== undefined
          ? { coveredAgencies: Math.trunc(num(get('covered_agencies'))!) }
          : {}),
        ...(num(get('covered_pct')) !== undefined ? { coveredPct: num(get('covered_pct'))! } : {}),
        ...(num(get('total_population')) !== undefined
          ? { totalPopulation: Math.trunc(num(get('total_population'))!) }
          : {}),
        source: PARTICIPATION_SOURCE,
        sourceUrl: provenanceUrl(PARTICIPATION_KEY),
        retrievedAt: nowIso,
        contentHash: sha256Json(stable).digest,
        datasetChecksum: participationChecksum,
        license: LICENSE,
  ...
  if (hateCrimeZip && existsSync(hateCrimeZip) && !process.argv.includes('--skip-storage-upload')) {
    ...
    await bucket.upload(hateCrimeZip, {
      destination: HC_STORAGE_PATH,
      metadata: { metadata: { sha256: hcChecksum } },
    });
    await bucket.file(`${HC_STORAGE_PATH}.sha256`).save(`${hcChecksum}  hate_crime.zip\n`);
    await bucket.upload(participationCsv!, {
      destination: PARTICIPATION_STORAGE_PATH,
      metadata: { metadata: { sha256: participationChecksum } },
    });
    await bucket
      .file(`${PARTICIPATION_STORAGE_PATH}.sha256`)
      .save(`${participationChecksum}  ucr_participation_1960_2024.csv\n`);
    console.log('Upload complete.');
  }

  const write = async (
    collectionName: string,
    docs: readonly { id: string; contentHash: string; createdAt: string }[],
  ) => {
  ...
  for (const [sourceId, checksum, storagePath] of [
    ...
    const acquisition = await recordDatasetAcquisition({
      ...
      retrievedAt: nowIso,
      snapshotStorageObject: `gs://${BUCKET}/${storagePath}`,
      parserVersion: PARSER_VERSION,
      httpStatus: 200,
    });
    console.log(
      `acquisition ${sourceId}: ${acquisition.sourceCaptureId ?? acquisition.retrievalEventId}`,
    );
  }
  ...
  if (agencyRegistry) {
    const acquisition = await recordDatasetAcquisition({
      firestore,
      registryEntry: agencyRegistry,
      contentHashHex: hcChecksum,
      retrievedAt: nowIso,
      parserVersion: PARSER_VERSION,
      httpStatus: 200,
    });
