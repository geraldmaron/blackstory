 ...
 *     packages/ops-data/scripts/dry-run-bulk-dc-sites.ts
 *
 *   node --conditions development --import tsx \
 *     packages/ops-data/scripts/dry-run-bulk-dc-sites.ts \
 *     --fixture=packages/ops-data/fixtures/discovery-candidates/bulk-dc-sites-2026-07-19.json
 */
...
const defaultCatalogDir = join(repoRoot, 'packages/ops-data/fixtures/national-catalog');
...
function renderMarkdown(report: BulkDcTriageReport): string {
  const lines: string[] = [
    ...
    '',
    'Regenerate: `node --conditions development --import tsx packages/ops-data/scripts/dry-run-bulk-dc-sites.ts`',
    '',
  ];
  return `${lines.join('\n')}\n`;
}
