 ...
 * It never changes published.*. Existing curated canonical values win; only demonstrable entity
...
} from '../src/postgres/canonical-convergence.js';
import { normalizePgConnectionString } from '@repo/data-access';
...
/**
 * --json emits one compact report object on stdout for machine callers; human output uses the
 * default format.
 */
...
async function assertNoCanonicalConflicts(
  ...
  }>(`SELECT id, entity_id FROM canonical.claims WHERE id = ANY($1::text[])`, [claimIds]);
...
async function applyPlan(client: pg.PoolClient, plan: CanonicalConvergencePlan): Promise<void> {
    ...
    await client.query(
      `
        ...
        ON CONFLICT (id) DO UPDATE SET
          display_name = CASE
            WHEN canonical.entities.display_name = canonical.entities.id
              THEN EXCLUDED.display_name
            ELSE canonical.entities.display_name
          END,
          kind = CASE
            WHEN canonical.entities.display_name = canonical.entities.id
              THEN EXCLUDED.kind
            ELSE canonical.entities.kind
          END,
          entity_class = COALESCE(canonical.entities.entity_class, EXCLUDED.entity_class),
          living_status = CASE
            WHEN canonical.entities.display_name = canonical.entities.id
          ...
          END,
          aliases = CASE
            WHEN canonical.entities.aliases = '[]'::jsonb THEN EXCLUDED.aliases
            ELSE canonical.entities.aliases
          END,
          status_history = CASE
            WHEN canonical.entities.status_history = '[]'::jsonb THEN EXCLUDED.status_history
            ELSE canonical.entities.status_history
          END,
          notability_basis = CASE
            WHEN canonical.entities.notability_basis = '[]'::jsonb THEN EXCLUDED.notability_basis
            ELSE canonical.entities.notability_basis
          END,
          sensitivity = CASE
            WHEN canonical.entities.sensitivity = '[]'::jsonb THEN EXCLUDED.sensitivity
            ELSE canonical.entities.sensitivity
          END,
          kind_detail = EXCLUDED.kind_detail || canonical.entities.kind_detail,
          updated_at = now()
      `,
  ...
  await forEachBatch(plan.sourceOrganizations, async (batch) => {
    await client.query(
      `
        INSERT INTO evidence.source_organizations (id, name, homepage)
  ...
  await forEachBatch(plan.sourceDomains, async (batch) => {
    await client.query(
      `
        INSERT INTO evidence.source_domains (id, organization_id, hostname)
  ...
  await forEachBatch(plan.evidenceSources, async (batch) => {
    await client.query(
      `
        INSERT INTO evidence.evidence_sources (
  ...
  await forEachBatch(plan.sourceItems, async (batch) => {
    await client.query(
      `
        INSERT INTO evidence.source_items (
  ...
  await forEachBatch(plan.evidenceRecords, async (batch) => {
    await client.query(
      `
        INSERT INTO evidence.evidence_records (
  ...
  await forEachBatch(plan.claims, async (batch) => {
    await client.query(
      `
        ...
        ON CONFLICT (id) DO UPDATE SET
          claim_class = COALESCE(canonical.claims.claim_class, EXCLUDED.claim_class),
          workflow_status = COALESCE(canonical.claims.workflow_status, EXCLUDED.workflow_status),
          publication_status = COALESCE(
            canonical.claims.publication_status,
            EXCLUDED.publication_status
          ),
          procedural_status = COALESCE(
            canonical.claims.procedural_status,
            EXCLUDED.procedural_status
          ),
          confidence = COALESCE(canonical.claims.confidence, EXCLUDED.confidence),
          research_coverage = COALESCE(
            canonical.claims.research_coverage,
            EXCLUDED.research_coverage
          ),
          verification = COALESCE(canonical.claims.verification, EXCLUDED.verification),
          updated_at = now()
      `,
      [JSON.stringify(batch)],
    );
  ...
  await forEachBatch(plan.relationships, async (batch) => {
    await client.query(
      `
        ...
        WHERE NOT EXISTS (
          SELECT 1
          FROM canonical.entity_relationships existing
...
async function verifyPlan(
  ...
  const result = await client.query<Verification>(
    `
      ...
      public_entities AS (
        SELECT re.*
        FROM published.release_entities re, active
      ...
      SELECT
        ...
        (
          ...
          LEFT JOIN canonical.entities e ON e.id = p.entity_id
          WHERE e.id IS NULL
        ) AS missing_canonical_entities,
        (
          SELECT count(*)::int
          FROM public_entities p
          JOIN canonical.entities e ON e.id = p.entity_id
        ...
        (
          ...
          WHERE p.lat IS NOT NULL AND p.lng IS NOT NULL
            AND NOT EXISTS (
              SELECT 1
              FROM canonical.entity_locations l
        ...
        (
          ...
          LEFT JOIN canonical.claims c ON c.id = p.id
          WHERE c.id IS NULL
        ) AS missing_planned_claims,
        (
          SELECT count(*)::int
          FROM planned_claims p
          JOIN canonical.claims c ON c.id = p.id
          WHERE c.current_version_id IS NULL
             OR NOT EXISTS (
               SELECT 1 FROM canonical.claim_versions v
        ...
        (
          ...
          WHERE NOT EXISTS (
            SELECT 1
            FROM canonical.claim_evidence_links link
...
/**
 * Records failed application attempts after rollback, on a separate query, so the failure audit
 * survives the aborted write transaction.
 */
  ...
  try {
    ...
    if (mode === 'apply') {
      try {
        ...
        console.error(
          'Failed to record the failure in audit.events (original error follows):',
          auditError,
        );
      }
    }
