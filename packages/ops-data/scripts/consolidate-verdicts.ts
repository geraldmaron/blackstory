...
function main(): void {
  ...
  const files = readdirSync(verdictDir)
    .filter((n) => /^verdict-\d+\.json$/.test(n))
    .sort();
  const verdicts: Verdict[] = [];
  for (const f of files) {
    const parsed = JSON.parse(readFileSync(join(verdictDir, f), 'utf8')) as {
      verdicts?: Verdict[];
    };
  ...
  for (const v of verdicts) {
    ...
    if (flag === 'none') publishNow.push(edge);
      ...
      reviewQueue.push(edge);
      bump(flagBreakdown, flag);
    }
  }

  writeFileSync(
    join(outDir, 'publish-edges.json'),
    JSON.stringify({ count: publishNow.length, edges: publishNow }, null, 2),
  );
  writeFileSync(
    join(outDir, 'review-queue.json'),
    JSON.stringify({ count: reviewQueue.length, edges: reviewQueue }, null, 2),
  );
  ...
  console.log(
    `total verdicts: ${verdicts.length} | accepted->publishNow: ${publishNow.length} | accepted->reviewQueue: ${reviewQueue.length} | rejected: ${rejected} | badType/self: ${badType.length}`,
  );
  console.log(
    'review-queue flag breakdown:',
    Object.fromEntries([...flagBreakdown.entries()].sort((a, b) => b[1] - a[1])),
  );
  console.log(
    'publishNow by type:',
    Object.fromEntries([...byType.entries()].sort((a, b) => b[1] - a[1])),
  );
  if (badType.length)
    console.log(
      'badType samples:',
      badType.slice(0, 5).map((v) => `${v.fromEntityId}->${v.toEntityId}:${v.finalType}`),
    );
  console.log(
