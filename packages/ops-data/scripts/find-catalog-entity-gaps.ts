 ...
 *    (packages/ops-data/scripts/lib/catalog-entity-match.ts) — the SAME
 ...
 *    @repo/ops-data, so a `@repo/ops-data → @repo/operator-cli` package
 ...
 *     under packages/ops-data/fixtures/discovery-candidates/, so they flow
 ...
 *     packages/ops-data/scripts/find-catalog-entity-gaps.ts [--max 50] [--apply]
 */
...
const discoveryDir = join(repoRoot, 'packages/ops-data/fixtures/discovery-candidates');
