...
const wrapper = join(repoRoot, 'packages/ops-data/scripts/run-enrichment-with-local-env.sh');
...
function loadPromotedIds(): Set<string> {
  const catalogDir = join(repoRoot, 'packages/ops-data/fixtures/national-catalog');
...
function loadAutoPromoteReport(stamp: string): {
  ...
  const fixturePath = join(
    repoRoot,
    'packages/ops-data/fixtures/national-catalog',
...
async function main(): Promise<void> {
  ...
  if (!skipEnrichment) {
    ...
    execSync(
      [
        ...
        join(repoRoot, 'packages/ops-data/scripts/auto-promote-corsair-keeps.ts'),
        '--run',
        join(cacheDir, 'rejudge-merged-enrichment-run.json'),
        '--subjects',
  ...
  if (newlyPromoted.length > 0) {
    execSync('DRY_RUN=1 APP_FIREBASE_ALLOW_PRODUCTION=1 node --conditions development --import tsx packages/ops-data/scripts/publish-national-catalog.ts', {
      cwd: repoRoot,
      stdio: 'inherit',
    });
    if (applyPublish) {
      execSync('APP_FIREBASE_ALLOW_PRODUCTION=1 node --conditions development --import tsx packages/ops-data/scripts/publish-national-catalog.ts', {
        cwd: repoRoot,
        stdio: 'inherit',
      });
      execSync(
