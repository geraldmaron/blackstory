 ...
 *   # Apply (writes only when every record validates AND every record clears the publish gate)
...
async function main(): Promise<void> {
  ...
  const runId = arg('run-id') ?? `manual_law_editorial_${new Date().toISOString().slice(0, 10).replace(/-/gu, '')}`;
  ...
  try {
    ...
    if (failures.length > 0 || bindFailures.length > 0) {
      process.exitCode = 1;
      if (!apply) return;
      console.log('Refusing to write: every record must validate and bind before any row is staged.');
      return;
    }
