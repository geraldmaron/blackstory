/** Validated record shapes shared by Postgres writers and domain adapters. */
import { z } from 'zod';
import {
  relationshipTypeSchema,
  type DbRelationshipType,
} from '@repo/schemas/relationship-vocabulary';
import {
  ACTOR_TYPES,
  AUDIT_EVENT_ACTIONS,
  OUTBOX_STATUSES,
...
export const authClaimFlagsSchema = z.object({
  ...
  app_role: z.enum(['admin', 'research', 'publication', 'security']).optional(),
});
...
/**
 * Coarse entity-class schema aligned with the domain entity-class resolver.
 */
...
export const canonicalEntitySchema = z.object({
  ...
  /** Coarse classification derived from kind by the shared entity-class resolver. */
  ...
  /** Derived output from deriveLivingStatus; never an independently sourced assertion. */
...
/**
 * Re-export the shared relationship vocabulary from @repo/schemas/relationship-vocabulary.
 * Direction and temporal write semantics live in @repo/domain RELATIONSHIP_TYPE_SEMANTICS. The
 * read parser warns on unknown values rather than silently inventing a known predicate.
 */
export { relationshipTypeSchema };

export type RelationshipTypeDoc = DbRelationshipType;
...
/**
 * Shared relationship workflow and publication vocabularies. Candidate represents an unreviewed
 * edge, not an accepted relationship.
 */
...
export const entityRelationshipSchema = z.object({
  ...
  createdAt: z.string().datetime(),
  updatedAt: z.string().datetime(),
  // Relationship workflow metadata; publication requires the applicable review gates.
 ...
 * (see `claimVersionSchema`), which is append-only at the persistence boundary.
 */
...
/**
 * Verification policy record aligned with the domain policy model. Binds subject classes and
 * predicates to intervals and eligible sources.
 */
...
/**
 * Operational verification state keyed by subject type/id, separate from editorial claim and
 * entity content.
 */
...
/**
 * Verification refresh proposal. Apply only through the normal review boundary; do not
 * overwrite public assertions directly.
 */
...
export const evidenceRecordSchema = z.object({
  ...
  /** Storage object reference; keep blob bytes outside the database. */
...
export const publicClaimProjectionSchema = z.object({
  ...
  independentLineageCount: z.number().int().nonnegative().optional(),
  /**
   * Whether the claim describes an index row or provides evidence about its subject. An absent
...
export const publicEntityProjectionSchema = z.object({
  ...
  /**
   * Auditable inclusion basis with evidence ids referring to this projection's claims. Absent
   * basis does not prove reviewed inclusion.
   */
  notabilityBasis: z.array(notabilityBasisRecordSchema).optional(),
  /**
   * Research-depth signal computed by publication from claims and citations. Missing coverage
   * remains detectable rather than being invented by the UI.
   */
  researchCoverage: z.enum(['minimal', 'partial', 'substantial']).optional(),
  /**
   * Actual release-build and verification timestamps. Never substitute render time for a
   * missing historical timestamp.
   */
  ...
  /**
   * Uncontrolled source tags. Use the classified topicIds, mentionedEntityIds and keywords
   * fields for their respective purposes.
   */
  topicTags: z.array(z.string().min(1)).default([]),
  /**
   * Theme identifiers must resolve against TOPIC_REGISTRY before facet use.
   */
  topicIds: z.array(z.string().min(1)).default([]),
  /**
   * Mentioned entity references, separate from themes. Resolve each value against canonical
   * entities before treating it as an edge; a raw imported tag is not an entity identifier.
   */
...
export const publicSearchIndexSchema = z.object({
  ...
  summary: z.string().optional(),
  /**
   * Uncontrolled source tags; never facet on them without taxonomy validation.
   */
  topicTags: z.array(z.string().min(1)).default([]),
  /**
   * Controlled theme identifiers for search facets, validated against TOPIC_REGISTRY.
   */
  topicIds: z.array(z.string().min(1)).default([]),
...
// Data-pack registry and import-decision records. Datasets begin disabled, batches begin
// pending and external ids remain namespaced until reviewed.
