...
test('convergence refuses malformed claims without substituting seed assertions', () => {
  assert.throws(
    () =>
      buildCanonicalConvergencePlan([
        row({ entity_id: 'ent_dc_landmark_listing_1975', claims: {} }),
      ]),
    /published claims must be an array/,
  );
