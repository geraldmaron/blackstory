...
function validateEvidence(
  ...
  if (record.evidence.length === 0) {
    errors.push('evidence is empty: a record with no independent source cannot clear the depth gate');
  }
...
function validateDisputes(record: AuthoredLawEntity, errors: string[], warnings: string[]): void {
  ...
  for (const dispute of disputes) {
    if (dispute.positions.length < 2) {
      errors.push(`dispute ${dispute.id} has fewer than two positions; that is a finding, not a dispute`);
    }
...
export function validateLawEntity(
  ...
  if (summary.length < LAW_ENTITY_SUMMARY_MIN_CHARS || summary.length > LAW_ENTITY_SUMMARY_MAX_CHARS) {
    errors.push(
  ...
  if (!Number.isFinite(record.place.lat) || !Number.isFinite(record.place.lng)) {
    errors.push('place.lat/place.lng must both be finite; the publisher skips a coordinate-less new record');
  }
