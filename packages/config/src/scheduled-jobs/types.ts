/**
 * BB-084 scheduled-job registry contracts — the framework other beads' recurring jobs plug
 * into. ADR-007 already picked the mechanism (Cloud Scheduler -> Cloud Tasks -> Cloud Run
 ...
 *
 * Binding operating principle (bd memories operating-principle-runs-itself-within-reason):
 * automation PROPOSES, humans/reviewers DISPOSE. Every job here either writes nothing public
 * at all, or is one of the two pre-approved mechanical + reversible exceptions declared below.
 */
...
/**
 * Environment every scheduled job runs in. BB-078 (ADR-012, landed in parallel with this bead)
 * names the research/admin pipeline project `blackbook-internal`; that ADR is committed, so we
 * reference the name directly rather than a numeric GCP project id (which BB-078/BB-079 may
...
export const ALLOWED_AUTOMATIC_PUBLIC_EFFECTS = [
  /** BB-083: repairs a dead citation link to point at an archived copy. Reversible: the repair
  ...
  /** BB-070: rebuilds a derived, regenerable artifact (map source tiles, search index) tied to
 ...
 *  bead below has not built the job body yet. Both are equally "no unregistered job can run";
...
export type ScheduledJobDefinition = {
  readonly id: string;
  /** Bead id (or ids) that owns this job's behavior, e.g. 'BB-037' or 'BB-039/BB-073'. */
   ...
   *  Cloud Tasks dispatches within the same window collapse to one effect (BB-018). */
  ...
  /** Required when rosterStatus is 'stub': which bead owns the still-to-be-built job body. */
