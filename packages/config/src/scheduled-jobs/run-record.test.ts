...
test('a failed run records an error summary', () => {
  ...
  const failed = failJobRun(started, {
    completedAt: '2026-07-17T04:00:05.000Z',
...
test('the in-memory store records, lists ascending by start, and finds the latest run per job', () => {
  ...
  store.record(
    startJobRun({ jobId: 'job-a', jobRunId: 'a-2', startedAt: '2026-07-17T02:00:00.000Z' }),
  );
  store.record(
    startJobRun({ jobId: 'job-a', jobRunId: 'a-1', startedAt: '2026-07-17T01:00:00.000Z' }),
  );
  store.record(
    startJobRun({ jobId: 'job-b', jobRunId: 'b-1', startedAt: '2026-07-17T01:30:00.000Z' }),
  );

  const historyA = store.listByJob('job-a');
  assert.deepEqual(
    historyA.map((run) => run.jobRunId),
    ['a-1', 'a-2'],
  );
