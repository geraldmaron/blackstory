/**
 * Field bag for the `movement` entity kind — Civil Rights Movement, Great Migration, Black
 * Power, Black Arts Movement, etc.
 *
 ...
 * `./entity-status.ts` `MOVEMENT_STATUSES`) — deliberately no `inactive`: a movement concludes,
 * it doesn't pause. `movement_significance` (`./entity-status.ts` `NOTABILITY_CRITERIA`) applies
 * both to entities associated with a movement AND to movement-kind entities themselves.
 */
export type MovementFields = {
  ...
  /** References jurisdiction ids the movement is primarily associated with. */
