 ...
 * onto the successor as the successor's current status. Canonical case: a historical place is
 * annexed or renamed — the historical entity keeps its own `historic`/closed statusHistory, and
 * the modern successor keeps its own, independently-derived current status (e.g. `active`).
 * Querying the successor via `successor_of` surfaces the predecessor's designation only as
 * linked historical context, never merged into or overwriting the successor's own status.
 *
 * Structural guardrail, not a runtime check alone: this module has NO function that reads or
 * writes an entity's `status`/`statusHistory` field directly (that stays
 ...
 * status derivation against the successor's OWN entity record, which this module never touches.
 * See `./succession.test.ts` for the explicit non-leakage proof against the place-annexation
 * scenario.
 */
