/**
 * Shared date-precision and era/decade bucket model (BB-090).
 *
 * Single source of truth for "how precise is this date" and "which decades does this span
 * touch" — consumed by the BB-086 FactRecord spec, BB-087 law status badges, BB-072 era
 * pre-filters, and the entity-lifecycle `statusHistory` records added in this bead (see
 * ./entity-status.ts). `deriveEraBuckets` replaces the embeddings-package-local
 * `deriveEraBucket` duplicate previously defined in `packages/firebase/src/embeddings/text.ts`
 * (that file now delegates its decade math to this module — see ADR-015).
 */

/**
 * How precisely a date is known. `circa` marks an approximate date at roughly year-level
 * confidence; it still buckets like `year` — the distinction is presentational (BB-095), not
 * arithmetic.
 */
