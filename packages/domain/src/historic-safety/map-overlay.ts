/**
 * BB-051/BB-070 map-overlay dignity-rule contract for the historic-safety layers (AC7). This
 * package owns no map UI (`apps/web/src/app/map/**`, `apps/web/src/components/map-experience/**`
 * are BB-051's) \u2014 this module is domain-side, stub-level hooks: the dignity RULES an overlay
 * config must satisfy, and a pure validator, so BB-051's actual map rendering can import and
 * enforce this contract without this package ever rendering a map itself.
 *
 * INTEGRATION POINT (documented, not live-wired \u2014 same convention as
 * ../geography/jurisdiction-refs.ts's own "INTEGRATION POINT" comment, and explicitly out of this
 * bead's file-ownership scope): BB-051's map layer toggle UI should call
 * `assertMapOverlayLayerConfigValid` on every historic-safety overlay entry before rendering it,
 * and should build each overlay's popup/off-ramp copy through `buildNarrativeOffRampLabel` below
 * rather than hand-writing overlay copy.
 */
import { HISTORIC_SAFETY_LAYER_LABELS, type HistoricSafetyLayerId } from './types.js';

/** The dignity rules every historic-safety map overlay must follow (bead design + BB-051 dignity
