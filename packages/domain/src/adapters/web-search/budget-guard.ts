 ...
 *
 * `@blap/domain` cannot depend on `@blap/security` at runtime -- `@blap/security`
 * already depends on `@blap/domain` (see ../internet-archive/shared/http-port.ts's doc
 ...
 * `@blap/domain` and `@blap/security`) is expected to back this port with
 * `evaluateDailyBudget` from `@blap/security` — the same way.
 *
 ...
 * of threshold/alert engine. The monthly spend ceiling is delegated entirely to the
 * injected evaluator so alerts and automated responses (BillingAlertPolicy, BudgetAutomatedResponse)
 * come from the real, already-audited policy matrix.
 */
...
/** A single budget policy row structurally identical to `DailyBudgetPolicy`. */
...
export type WebSearchCampaignBudgetPolicy = {
  ...
  /** Monthly USD-cent spend ceiling fed to the injected evaluator. */
  readonly monthlySpendCapUsdCents: number;
  readonly costPerQueryUsdCents: number;
  /** budget category key the monthly check is recorded under (e.g. "research_campaign"). */
