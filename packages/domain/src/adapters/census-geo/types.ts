/**
 * U.S. Census Bureau Geocoder API request/response types (BB-050) — the LIVE, per-request
 * geocoding service at geocoding.geo.census.gov, distinct from the bulk TIGER/Gazetteer
 * reference-file contract in `./contract.ts` (BB-091's annual state/county load). Both are
 ...
 * repo's own BB-025 `geocoding` endpoint-class quota (packages/security/src/rate-limits.ts) is
