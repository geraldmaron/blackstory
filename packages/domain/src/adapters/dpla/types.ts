/**
 * DPLA v2 discovery adapter types.
 *
 * IMPORTANT — endpoint/shape-churn tolerance: DPLA's aggregation program is transitioning to
 * Cleveland Public Library starting July 2026. This adapter therefore does NOT assume
 * `api.dp.la/v2`'s current response shape is stable. Every field is read defensively
 * (`client.ts`'s `parseDplaDoc`): known alternate field names/locations are tried in order, a
 * record that is missing the bare minimum (an id and a title) is rejected (recorded, not
 * thrown — one malformed/renamed field never poisons a whole batch), and no code path assumes
 * a field is present without a runtime type check. If DPLA v2 is retired in favor of a new
 * Cleveland-operated endpoint, only `client.ts`'s field-extraction functions and
 * `fetch-search.ts`'s URL builders should need to change — `normalizer.ts` and the contract
