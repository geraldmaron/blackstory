/**
 * Tests for the BB-054 missing-perspective indicator derivation.
 */
...
test('returns no indicators once harm is balanced by at least two other dimensions', () => {
  assert.deepEqual(
    deriveMissingPerspectiveIndicators(['harm', 'community', 'resistance']),
    [],
  );
});
...
test('returns indicators for every undocumented balancing dimension when harm-only', () => {
  ...
  assert.ok(indicators.every((indicator) => /reflects the current state of research/.test(indicator.note)));
});
...
test('excludes already-documented balancing dimensions from the indicator list when coverage is thin', () => {
  ...
  assert.equal(indicators.some((indicator) => indicator.dimension === 'community'), false);
