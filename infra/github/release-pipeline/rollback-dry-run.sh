...
# Public web rollback uses Vercel prior-SHA promotion or redeployment.
set -euo pipefail
...
echo "  4. Reconcile published.active_release with the restored database and compatible clients"
...
echo "  Follow docs/runbooks/backup-restore.md and record executed recovery evidence in artifacts/recovery/latest.json"
echo '  node scripts/launch/evaluate-beta-gate.mjs --evaluator "$USER" --output artifacts/recovery/launch-decision.json'
