...
6. Emulator: local Auth uses `demo-repo` via `pnpm firebase:emulators` — never point local clients at production Auth without the production break-glass flag in `@repo/firebase`.
