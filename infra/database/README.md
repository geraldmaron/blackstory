...
> `infra/firebase/` + `@black-book/firebase` for active work.
...
| Path | Status |
|------|--------|
| Firestore model / rules | **Active** — `infra/firebase/`, `@black-book/firebase` |
...
`@black-book/data-access` — Firestore access guards (primary) + parked Postgres/SQL Connect
